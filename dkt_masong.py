...
Original file is located at
    https://colab.research.google.com/drive/11xT0oKyThAxn5ipuoJDmBqDKAlIPVCR5
"""
...
tf.flags.DEFINE_string("train_data_path", '/content/drive/My Drive/data/0910_a_train.csv', "Path to the training dataset")
tf.flags.DEFINE_string("test_data_path", '/content/drive/My Drive/data/0910_b_test.csv', "Path to the testing dataset")
...
class DKTModel(object):
    ...
    def __init__(self, is_training, config):
        ...
        print("^^^^^^^^^^^^^^^^^^^^{}".format(final_hidden_size))

        final_hidden_size = int(final_hidden_size)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=session_conf) as session:
            ...
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = DKTModel(is_training=True, config=config)
            # testing model
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mtest = DKTModel(is_training=False, config=eval_config)
